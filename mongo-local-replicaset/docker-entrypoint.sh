...
# Based on https://hub.docker.com/r/luamas/docker-mongo-local-replicaset
# with some of its connectivity and auth issues resolved
...
trap 'echo "KILLING CLUSTER"; kill $DB1_PID $DB2_PID $DB3_PID; wait $DB1_PID; wait $DB2_PID; wait $DB3_PID' SIGINT SIGTERM EXIT
